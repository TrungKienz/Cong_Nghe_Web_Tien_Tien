...
const getMarkComment = async (req, res) => {
    try {
      ...
      const resultComment = validComment.slice(Number(index), Number(index) + Number(count));
        const resultMark = validMark.slice(Number(index), Number(index) + Number(count));
      ...
      switch (err.message) {
        case "params":
          ...
          errorResponse.message = statusMessage.PARAMETER_VALUE_IS_INVALID;
          break;
        case "notfound":
          ...
          break;
        case "blocked":
          errorResponse.code = statusCode.ACTION_HAS_BEEN_DONE_PREVIOUSLY_BY_THIS_USER;
          errorResponse.message = statusMessage.ACTION_HAS_BEEN_DONE_PREVIOUSLY_BY_THIS_USER;
          break;
        case "authorblock":
        case "userblock":
    ...
    try {
      ...
      if (mark_id && (type === '1' || type === '0')) {
        ...
        return res.status(200).json({
          code: statusCode.OK,
          message: statusMessage.OK,
          data: postInfor.mark_list.slice(Number(index), Number(index) + Number(count)),
        });
        ...
        return res.status(200).json({
          code: statusCode.OK,
          message: statusMessage.OK,
          data: postInfor.comment_list.slice(Number(index), Number(index) + Number(count)),
        });
      }
    } catch (error) {
      console.error(error);
