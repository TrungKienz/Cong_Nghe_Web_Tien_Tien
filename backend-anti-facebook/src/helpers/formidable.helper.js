...
let parse = (req, postData) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (
            videoList &&
            videoList[0].size < 1024 * 1024 &&
            videoList[0].size > 10 * 1024 * 1024
        ) {
                    ...
                    if (
                        ...
                        getVideoDurationInSeconds(file.path)
                            .then((duration) => {
                                if (duration < 1 || duration > 10) {
                                    console.log(
                                        'thời lượng video <1s hoặc lớn hơn 10s'
                                    );
                                    return reject(
                                        statusCode.FILE_SIZE_IS_TOO_BIG
                                    );
                                }
                            })
                            .catch((err) => {
...
let parseInfo = (req) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (
            (files.avatar && files.avatar[0].mimetype.slice(0, 5) != 'image') ||
            (files.cover_image &&
                files.cover_image[0].mimetype.slice(0, 5) != 'image')
        ) {
        ...
        try {
            ...
            if (files.cover_image) {
                resCoverImage = await cloud.upload(
                    files.cover_image[0],
                    'image'
                );
            }
